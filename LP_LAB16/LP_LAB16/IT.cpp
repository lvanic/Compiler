...
#pragma region Literals
			...
			*(idStream) << '|' << std::setw(15) << "Идентификатор: " << std::setw(15) << "Тип данных: " << "|" << std::setw(50) << "Значение: " << '|' << std::setw(20) << "Длина строки: " << '|' << "Первое вхождение: " << '|' << std::endl;
			...
			for (int i = 0; i < this->size; i++)
			{
				if (this->table[i].idtype == IT::IDTYPE::L)
				{
					...
					case 1:
					{
						*(idStream) << '|' << std::setw(24) << this->table[i].id << '|' << std::setw(24) << "INT " << '|' << std::setw(60) << this->table[i].value.vint << '|' << std::setw(30) << "-" << '|' << this->table[i].firstApi << std::endl;
						break;
					}
					case 2:
					{
						*(idStream) << '|' << std::setw(23) << this->table[i].id << '|' << std::setw(23) << "STR" << '|' << std::setw(60 - strlen(this->table[i].value.vstr.str)) << this->table[i].value.vstr.str << '|' << std::setw(30) << (int)this->table[i].value.vstr.len << '|' << this->table[i].firstApi << std::endl;
						break;
					}
					}
				}
